...
<div class="wrap scm-wrap">
    <h1><?php echo $rule['id'] ? esc_html__( 'Edit Rule', 'schema-control-pro' ) : esc_html__( 'Add Rule', 'schema-control-pro' ); ?></h1>

    <?php if ( ! empty( $_GET['updated'] ) ) : ?><div class="notice notice-success"><p><?php esc_html_e( 'Rule saved.', 'schema-control-pro' ); ?></p></div><?php endif; ?>
    ...
    <div class="notice notice-success"><p>
        ...
        if ( $rule_label_for_notice ) {
            ...
            printf( esc_html__( 'Schema saved for rule: %s', 'schema-control-pro' ), '<strong>' . esc_html( $rule_label_for_notice ) . '</strong>' );
        } else {
            esc_html_e( 'Schema saved.', 'schema-control-pro' );
        }
        ?>
    </p></div>
    <?php endif; ?>
    <?php if ( ! empty( $_GET['schema_deleted'] ) ) : ?><div class="notice notice-success"><p><?php esc_html_e( 'Schema deleted.', 'schema-control-pro' ); ?></p></div><?php endif; ?>
    <?php if ( ! empty( $_GET['rule_error'] ) ) : ?><div class="notice notice-error"><p><strong><?php esc_html_e( 'Rule could not be saved:', 'schema-control-pro' ); ?></strong> <?php echo esc_html( wp_unslash( $_GET['rule_error'] ) ); ?></p></div><?php endif; ?>
    ...
    <div class="notice notice-warning">
        <p><strong><?php esc_html_e( 'Schema access blocked', 'schema-control-pro' ); ?></strong></p>
        <p><?php
        if ( ! empty( $_GET['schema_id'] ) ) {
            printf(
                /* translators: %d: schema ID */
                esc_html__( 'Schema #%d cannot be edited here — it does not belong to this rule. A blank form has been loaded instead.', 'schema-control-pro' ),
                (int) $_GET['schema_id']
            );
        }
        ?></p>
    ...
    <?php if ( ! empty( $runtime_notices ) ) : ?>
        <div class="notice scm-notice-runtime">
            <p><strong><?php esc_html_e( 'Runtime issue detected on last frontend render', 'schema-control-pro' ); ?></strong>
    ...
    <?php if ( ! empty( $rule_diagnostics ) ) : ?>
        ...
        <?php if ( $has_rule_issues ) : ?>
            <div class="notice <?php echo ! empty( $rule_diagnostics['errors'] ) ? 'notice-error' : 'notice-warning'; ?> scm-notice-rule-diag">
                <p><strong><?php esc_html_e( 'Rule-level diagnostics (all schemas combined)', 'schema-control-pro' ); ?></strong></p>
    ...
    <div class="scm-grid scm-grid-top">
        <div class="scm-card">
            <h2><?php esc_html_e( 'Rule', 'schema-control-pro' ); ?></h2>
            <form method="post">
                <?php wp_nonce_field( 'scm_save_rule' ); ?>
                <input type="hidden" name="rule_id" value="<?php echo esc_attr( $rule['id'] ); ?>">
                <table class="form-table">
                    <tr>
                        <th><label for="label"><?php esc_html_e( 'Label', 'schema-control-pro' ); ?></label></th>
                        <td><input class="regular-text" type="text" name="label" id="label" value="<?php echo esc_attr( $rule['label'] ); ?>" required></td>
                    </tr>
                    <tr>
                        <th><label for="target_type"><?php esc_html_e( 'Target type', 'schema-control-pro' ); ?></label></th>
                        <td>
                            <select name="target_type" id="target_type">
                                <option value="home" <?php selected( $rule['target_type'], 'home' ); ?>><?php esc_html_e( 'Home (front page or blog index)', 'schema-control-pro' ); ?></option>
                                <option value="front_page" <?php selected( $rule['target_type'], 'front_page' ); ?>><?php esc_html_e( 'Front page (static only)', 'schema-control-pro' ); ?></option>
                                <option value="exact_url" <?php selected( $rule['target_type'], 'exact_url' ); ?>><?php esc_html_e( 'Exact URL', 'schema-control-pro' ); ?></option>
                                <option value="exact_slug" <?php selected( $rule['target_type'], 'exact_slug' ); ?>><?php esc_html_e( 'Exact slug', 'schema-control-pro' ); ?></option>
                                <option value="post_type" <?php selected( $rule['target_type'], 'post_type' ); ?>><?php esc_html_e( 'Post type (all singulars)', 'schema-control-pro' ); ?></option>
                                <option value="post_type_archive" <?php selected( $rule['target_type'], 'post_type_archive' ); ?>><?php esc_html_e( 'Post type archive', 'schema-control-pro' ); ?></option>
                                <option value="category" <?php selected( $rule['target_type'], 'category' ); ?>><?php esc_html_e( 'Category archive', 'schema-control-pro' ); ?></option>
                                <option value="tag" <?php selected( $rule['target_type'], 'tag' ); ?>><?php esc_html_e( 'Tag archive', 'schema-control-pro' ); ?></option>
                                <option value="taxonomy_term" <?php selected( $rule['target_type'], 'taxonomy_term' ); ?>><?php esc_html_e( 'Taxonomy term archive', 'schema-control-pro' ); ?></option>
                                <option value="author" <?php selected( $rule['target_type'], 'author' ); ?>><?php esc_html_e( 'Author page', 'schema-control-pro' ); ?></option>
                            </select>
                        </td>
                    </tr>
                    <tr class="scm-target-value-row">
                        <th><label for="target_value"><?php esc_html_e( 'Target value', 'schema-control-pro' ); ?></label></th>
                        <td>
                            ...
                            <p class="description" id="scm-target-help">
                                <?php esc_html_e( 'Home and Front page do not need a value.', 'schema-control-pro' ); ?>
                                <?php esc_html_e( 'For post_type / post_type_archive / category / tag: enter the slug (e.g. post, movies, news).', 'schema-control-pro' ); ?>
                                <?php esc_html_e( 'For taxonomy_term: use taxonomy:term-slug (e.g. genre:fiction).', 'schema-control-pro' ); ?>
                                <?php esc_html_e( 'For author pages: use the user_nicename.', 'schema-control-pro' ); ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="mode"><?php esc_html_e( 'Mode', 'schema-control-pro' ); ?></label></th>
                        <td>
                    ...
                    <tr class="scm-replaced-types-row">
                        ...
                        <td>
                            <p class="description"><?php esc_html_e( 'Use this only for structural replacements or specific overrides. Structural types such as BreadcrumbList, Person and Organization may require @id rewiring.', 'schema-control-pro' ); ?></p>
                    ...
                    <tr>
                        <th><label for="rule_priority"><?php esc_html_e( 'Priority', 'schema-control-pro' ); ?></label></th>
                        <td>
                            <input class="small-text" type="number" name="priority" id="rule_priority" value="<?php echo esc_attr( $rule['priority'] ?? 100 ); ?>">
                            <p class="description"><?php esc_html_e( 'Higher number = evaluated first. Default 100.', 'schema-control-pro' ); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th><?php esc_html_e( 'Status', 'schema-control-pro' ); ?></th>
                        <td><label><input type="checkbox" name="is_active" value="1" <?php checked( ! empty( $rule['is_active'] ) ); ?>> <?php esc_html_e( 'Active', 'schema-control-pro' ); ?></label></td>
                    </tr>
                </table>
                <p><button class="button button-primary" name="scm_save_rule" value="1"><?php esc_html_e( 'Save Rule', 'schema-control-pro' ); ?></button></p>
            </form>
        </div>

        <div class="scm-card">
            <h2><?php esc_html_e( 'Rule summary', 'schema-control-pro' ); ?></h2>
            <ul class="scm-summary-list">
                <li><strong><?php esc_html_e( 'AIOSEO', 'schema-control-pro' ); ?>:</strong> <span id="scm-summary-aioseo"><?php echo esc_html( $rule_summary['aioseo_status'] ); ?></span></li>
                <li><strong><?php esc_html_e( 'Custom schemas', 'schema-control-pro' ); ?>:</strong> <span id="scm-summary-custom-count"><?php echo esc_html( $rule_summary['custom_count'] ); ?></span></li>
                <li><strong><?php esc_html_e( 'Replacements', 'schema-control-pro' ); ?>:</strong> <span id="scm-summary-replacements"><?php echo esc_html( empty( $rule_summary['replacements'] ) ? '—' : implode( ', ', $rule_summary['replacements'] ) ); ?></span></li>
                <li><strong><?php esc_html_e( 'Expected output', 'schema-control-pro' ); ?>:</strong> <span id="scm-summary-output"><?php echo esc_html( $rule_summary['output'] ); ?></span></li>
            </ul>
            <div class="scm-help-box">
                <p><strong><?php esc_html_e( 'Usage hints', 'schema-control-pro' ); ?></strong></p>
                <ul>
                    <li><?php esc_html_e( 'Use AIOSEO + Custom for additive schemas like FAQPage, HowTo or Service.', 'schema-control-pro' ); ?></li>
                    <li><?php esc_html_e( 'Use Override when replacing structural nodes such as BreadcrumbList, Person or Organization.', 'schema-control-pro' ); ?></li>
                    <li><?php esc_html_e( 'Use Custom only when you want full control of the page graph.', 'schema-control-pro' ); ?></li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div class="scm-card">
        <h2>
            <?php esc_html_e( 'Schemas attached to this rule', 'schema-control-pro' ); ?>
            <?php if ( ! empty( $rule['label'] ) ) : ?>
                <span class="scm-rule-owner-badge"><?php echo esc_html( $rule['label'] ); ?></span>
            <?php endif; ?>
        </h2>
        <?php if ( ! $rule['id'] ) : ?>
            <p><?php esc_html_e( 'Save the rule first to attach schemas.', 'schema-control-pro' ); ?></p>
        <?php elseif ( empty( $schemas ) ) : ?>
            <p><?php esc_html_e( 'No schemas yet.', 'schema-control-pro' ); ?></p>
        <?php else : ?>
    ...
    <div class="scm-card scm-card-full">
        <h2>
            <?php echo $edit_schema['id'] ? esc_html__( 'Edit Schema', 'schema-control-pro' ) : esc_html__( 'Add Schema', 'schema-control-pro' ); ?>
            <?php if ( ! empty( $rule['label'] ) ) : ?>
                <span class="scm-rule-owner-badge"><?php
                    printf(
                        /* translators: %s: rule label */
                        esc_html__( 'Rule: %s', 'schema-control-pro' ),
                        esc_html( $rule['label'] )
                    );
                ?></span>
            <?php endif; ?>
            ...
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php if ( 'author' === $rule['target_type'] ) : ?>
                            <p class="description"><?php esc_html_e( 'For author pages, Person usually behaves as a structural node and should keep a stable @id when possible.', 'schema-control-pro' ); ?></p>
                        <?php endif; ?>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <p class="description"><?php esc_html_e( 'You can paste a full JSON-LD object, a list of nodes, or an object with @graph. The plugin normalizes to a final @context + @graph structure.', 'schema-control-pro' ); ?></p>
                        <p><button type="button" class="button" id="scm-validate-json">Validate JSON</button> <span id="scm-json-status"></span></p>
                    </td>
                </tr>
                <tr>
                    <th><?php esc_html_e( 'Normalized preview', 'schema-control-pro' ); ?></th>
                ...
                <tr>
                    <th><?php esc_html_e( 'Diagnostics', 'schema-control-pro' ); ?></th>
                    <td>
                        <div class="scm-diagnostics">
                            <p><strong><?php esc_html_e( 'Node count:', 'schema-control-pro' ); ?></strong> <?php echo esc_html( $diagnostics['node_count'] ); ?></p>
                            <p><strong><?php esc_html_e( 'Detected types:', 'schema-control-pro' ); ?></strong> <?php echo esc_html( empty( $diagnostics['types'] ) ? '—' : implode( ', ', $diagnostics['types'] ) ); ?></p>
                            <p><strong><?php esc_html_e( 'Domains in @id:', 'schema-control-pro' ); ?></strong> <?php echo esc_html( empty( $diagnostics['domains'] ) ? '—' : implode( ', ', $diagnostics['domains'] ) ); ?></p>

                            <?php // ── Critical errors ───────────────────────────────────────── ?>
                            <?php if ( ! empty( $diagnostics['errors'] ) ) : ?>
                                <p class="scm-severity-label scm-severity-critical"><?php esc_html_e( 'Critical errors', 'schema-control-pro' ); ?></p>
                                <ul class="scm-warning-list scm-error-list">
                            ...
                            <?php if ( ! empty( $diagnostics['structural_warnings'] ) ) : ?>
                                <p class="scm-severity-label scm-severity-structural"><?php esc_html_e( 'Structural warnings', 'schema-control-pro' ); ?></p>
                                <ul class="scm-warning-list scm-structural-warning-list">
                            ...
                            <?php if ( ! empty( $diagnostics['semantic_warnings'] ) ) : ?>
                                <p class="scm-severity-label scm-severity-semantic"><?php esc_html_e( 'Semantic warnings', 'schema-control-pro' ); ?></p>
                                <ul class="scm-warning-list scm-semantic-warning-list">
                            ...
                            <?php if ( empty( $diagnostics['errors'] ) && empty( $diagnostics['warnings'] ) ) : ?>
                                <p class="scm-ok"><?php esc_html_e( 'No graph issues detected in this schema.', 'schema-control-pro' ); ?></p>
                            <?php endif; ?>
                        </div>
                    </td>
                </tr>
